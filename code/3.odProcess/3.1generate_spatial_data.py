...
def run():
    ...
    mapping_df.to_csv(os.path.join(config.CENSUS_DIR, f"nodeid_geocode_mapping_{config.REGION}.csv"), index=False)
    ...
    adj_df.to_csv(os.path.join(config.CENSUS_DIR, f"adjacency_matrix_bycar_m_{config.REGION}.csv"))
