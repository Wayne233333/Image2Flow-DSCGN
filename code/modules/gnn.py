...
class GATLayer(nn.Module):
    ...
    def edge_feat_func(self, edges):
        return {'t': self.fcV(edges.data['d'].to(edges.src['z'].device))}
    ...
    def reduce_func(self, nodes):
        ...
        # 邻居节点传递过来的信息 (对应拉普拉斯/邻接传播项)
        z_neighbor = torch.sum(alpha * nodes.mailbox['z'], dim=1)
        
        # 节点自身的信息
        z_i = nodes.data['z_i']
        
        # 【DSGNN 核心修改】：获取节点势能 v_i，并作用于节点自身特征
        # 模拟哈密顿算子 H = L + V 的对角线作用
        if 'v' in nodes.data:
            v_i = nodes.data['v'].unsqueeze(-1)  # 扩展维度以匹配特征维度 (batch_size, 1)
            # 势能调节自身状态的保留程度
            h = self.activation(z_neighbor + v_i * z_i)
        else:
            h = self.activation(z_neighbor + z_i)
